 ...
 */
public class VideoCatchAdapter extends BaseAdapter {
    private Context context;
    private List<VideoPro> mVideoModels;

    public VideoCatchAdapter(Context context, List<VideoPro> videoModels) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        if (convertView == null) {
            convertView = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_video_catch, parent, false);
        ...
        //视频对象
        VideoPro item = mVideoModels.get(position);
        ...
        // 设置看到多少集
        TextView videoItemText = convertView.findViewById(R.id.video_item_text);

        videoItemText.setText("看到第" + item.getNum() + "集");

        // 设置更新到多少进度
        TextView textDesc = convertView.findViewById(R.id.video_desc);

        textDesc.setText("更新至" + mVideoModels.size() + "集 | 完结状态");
